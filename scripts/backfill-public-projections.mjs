...
// Temporary one-shot: consolidate Nutcracker into one ballet entry + Act II movements.
{
  const here = dirname(fileURLToPath(import.meta.url));
  const r = spawnSync(process.execPath, [join(here, 'seed-season-repertoire.mjs')], {
    env: process.env,
    stdio: 'inherit',
  });
